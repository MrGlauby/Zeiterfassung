...
export class TimeDataAccessService {
  ...
  public addHours(userId: number, hour: Hour): Observable<Hour> {
    const currentHours = this.userHours$$.getValue(); // Aktuellen Wert holen
    if (!currentHours[userId]) {
      currentHours[userId] = [];
    }
    currentHours[userId].push(hour); // Neuen Wert hinzufügen
    this.userHours$$.next({ ...currentHours }); // Aktualisierten Wert setzen
    return of(hour);
  }
